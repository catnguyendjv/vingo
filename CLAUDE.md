...
Trạng thái: P0 (schema, RLS, web đọc/học, migrate dữ liệu cũ, backup cron, UI redesign), P1 (`apps/mcp` study-kit-mcp chạy localhost + `packages/core` + skill `skills/study-kit` + `/settings` pairing), P1.5 (SRS `/review`, spec `2026-09-08-vingo-p15-srs-design.md`) và P2 (video local, YouTube player, share community, polish mobile, service worker — spec `2026-09-08-vingo-p2-design.md`) code xong; P2 còn kiểm tay trên điện thoại thật. Chưa làm: phase deploy (cloud provisioning Task 17, Supabase Pro, deploy MCP — đã quyết định lùi, không đề xuất lại trừ khi được yêu cầu).
...
pnpm e2e                                      # Playwright; cần Supabase local + dev server :3000 đang chạy (pwa.spec.ts tự skip)
pnpm e2e review.spec.ts                       # một file e2e — KHÔNG thêm `--` (pnpm 10 truyền nguyên văn → Playwright chạy cả suite)
NEXT_DIST_DIR=.next-e2e pnpm --filter web build                         # build production song song dev server (distDir riêng)
NEXT_DIST_DIR=.next-e2e pnpm --filter web exec next start -p 3200       # chạy nền; cổng 3097–3106 máy dev bị MCP nội bộ chiếm
E2E_PWA=1 E2E_BASE_URL=http://localhost:3200 pnpm e2e pwa.spec.ts       # e2e service worker/offline; xong thì dừng next start
...
**Trang học.** `components/study/StudyView.tsx` giữ toàn bộ state và mutation (tiến độ câu, từ đã thuộc, A–B repeat, tốc độ, edit mode) và một `<video>` duy nhất; các component con (`CueList/CueItem`, `VocabPanel`, `StudyControls`, `MobileDock`) chỉ nhận props. Desktop và mobile là **một cây DOM** phân nhánh bằng `lg:` (sticky video + tab Câu/Từ vựng + dock đáy trên mobile), không render hai cây; variant `land:` (`@custom-variant` trong `globals.css`: landscape + cao ≤ 500px) cho layout 2 cột khi xoay ngang điện thoại, dock ẩn. Khối video là `components/study/VideoFrame.tsx` chọn `PlayerAdapter` (`lib/player.ts`) theo `video_provider`: `Html5PlayerAdapter` cho `storage` (signed URL) và `local` (object URL từ file người học tự chọn qua `LocalVideoSource`, video không lên cloud, desktop Chromium nhớ file handle trong IndexedDB — `lib/local-video.ts`), `YouTubePlayerAdapter` (`lib/player-youtube.ts`, IFrame API tự viết, không autoplay, poll 250ms + nội suy). Cue-sync thuần trong `lib/cues.ts` có unit test. Signed URL video/thumbnail cache theo `${userId}:${path}` trong `lib/video.ts` — không bỏ userId khỏi key. Share community chỉ là UPDATE `visibility` trên RLS sẵn có (`ShareButton`, dialog xác nhận riêng cho nguồn `zoom`).

**Service worker (P2).** `apps/web/public/sw.js` viết tay, tĩnh, không qua bundler (quyết định P2-6: không serwist vì không hỗ trợ Turbopack). Đổi hằng `VERSION` mỗi lần sửa file để activate xoá cache cũ. Navigate cùng origin: network-first 4s → cache → `/offline`; `/_next/static`, icon, `fonts.gstatic.com`: cache-first; bỏ qua Supabase (origin nhận qua `sw.js?supabase=`), `/auth/*`, YouTube, request có `Range` — không offline video, không cache dữ liệu. `components/layout/ServiceWorkerRegistrar.tsx` (trong `app/layout.tsx`) chỉ đăng ký khi `NODE_ENV === "production"`, có toast "Có bản mới" thay vì tự reload. `middleware.ts` loại `/offline`, `/sw.js`, `/manifest.webmanifest` khỏi matcher (SW không cache được redirect về `/login`); `app/offline/page.tsx` là `force-static`, không gọi Supabase. `next.config.ts` đọc `NEXT_DIST_DIR` để build production ra thư mục khác `.next` khi dev đang chạy.
...
**Kiểm thử.** Vitest: `.test.ts` thuần + `.test.tsx` (jsdom, testing-library, đặt `// @vitest-environment jsdom` đầu file). Playwright `e2e/smoke.spec.ts` dùng `data-testid` (`theme-toggle`, `cue-item`, `flashcard` + `data-known`, `mobile-dock`, `dict-table`/`dict-list`) và phải idempotent (trả lại trạng thái đã đổi); `review.spec.ts` giả định dev user không có thẻ ôn tập tồn — nếu lần chạy trước hỏng giữa chừng, xoá `review_cards` của `cat@vingo.local` rồi chạy lại. `e2e/pwa.spec.ts` chỉ chạy khi `E2E_PWA=1` trên production build (xem mục Lệnh), suite thường tự skip. GitHub Actions `db-backup.yml` dump Postgres hằng đêm (cần secret `SUPABASE_DB_URL`).
